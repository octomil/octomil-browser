...
describe("ModelManager", () => {
  ...
  describe("resolveModelUrl", () => {
    it("uses the model string directly when it is a URL", async () => {
      ...
      expect(url).toBe("https://REDACTED_URL/test.onnx");
    });
    ...
    it("resolves a model name via the registry", async () => {
      const registryResponse = {
        ...
        format: "onnx",
        sizeBytes: 1024,
        url: "https://REDACTED_URL/models/sentiment-v1.onnx",
      };
      ...
      expect(url).toBe("https://REDACTED_URL/models/sentiment-v1.onnx");
      expect(fetch).toHaveBeenCalledWith(
        "https://REDACTED_URL/api/v1/models/sentiment-v1/metadata",
  ...
  describe("load", () => {
    it("returns cached data when available", async () => {
      ...
      stored.set("https://REDACTED_URL/test.onnx", data);
  ...
  describe("isCached / clearCache", () => {
    it("reports cached status", async () => {
      ...
      // Simulate caching.
      const stored = new Map<string, ArrayBuffer>();
      stored.set("https://REDACTED_URL/test.onnx", fakeOnnxBuffer());
      cache = createMockCache(stored);
